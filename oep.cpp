...
class Train
{
    ...
    void putdata()
    {
        cout<<train_name<<" "<<" "<<arr_t<<" "<<source<<" "<<dest<< " "<<dep_t<<" "<<train_no<<" ";
        for(i,0,6)cout<<fare[i]<<" ";
        for(i,0,6)cout<<availability[i]<<" ";
        for(i,0,6)cout<<c_cat[i]<<" ";
        cout<<endl;
        cout<<"--------------------------------------------------\n";
...
void matchTrains(User query)
    {
        ...
        if(count>0)
        {
            ...
            while(1)
            {
                ...
                cout<<"waiting: "<<waiting<<endl<<" Confirmed:"<<t[index-1].availability[category-1]<<endl;
                break;
            }
        }
        else
