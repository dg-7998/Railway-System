...
class Train
{
    ...
    void getdata()
    {
        ...
        cout<<"enter fares:";
        for(i,0,6)if(c_cat[i]==1)cin>>fare[i]; else fare[i]=-1;
    ...
    void insert()
    {
        ...
        cin>>n;
        fstream f1;
        f1.open(filename,ios::out|ios::app|ios::binary);
        Train t[n];
        for(i,0,n)
            {
                t[i].getdata();
            ...
            if(obj.train_no==match.train_no)
                {
                    ...
                    f1.seekp(pos-sizeof(obj));
                    //----------------update the matching train the file-------------------------
                    f1.write((char*)&match,sizeof(match));
                }
        }
...
void matchTrains(User query)
    {
        ...
        if(count>0)
        {
            ...
            while(1)
            {
                ...
                if(waiting>0)
                {
                    cout<<"Do you want to continue with "<<passengers<<"ticket to be booked? (1.YES 2.NO)";
                    cin>>ch;
                    if(ch==1)cout<<"Waiting: "<<waiting<<endl<<passengers<<"Confirmed.Ticket Booked"<<endl;
                    if(ch==2)
                }
